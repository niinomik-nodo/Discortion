...
def get_message(message):
    ...
    try:
        ...
        err_message_username, err_annaounce_username = check_user(
            get_messemger_name)
...
def check_command(command):
    ...
    elif int(command[6:]) > 327:
        err_message = error_dict["message"]["command"]["excess"]
        err_annaounce = error_dict["annaounce"]["command"]["excess"]
...
def check_user(messemger_name):
    ...
    if (not messemger_name[-1].isdigit()) | (messemger_name[-1] == "_"):
        ...
        err_message = error_dict["message"]["user"]["dbidkey"]
        err_annaounce = error_dict["annaounce"]["user"]["dbidkey"]
    return err_message, err_annaounce
...
def main():
    ...
    TOKEN = "MTA5ODEwNDI1OTkyNjU3MzA3Ng.Gc02V0.f9p0lk0nVBAX9rfJvi-Ad56feQ3Rvt4yXxd-Z8"
    ...
    async def on_message(message):
        ...
        if message.channel.id == int(CHANNEL_ID):
            ...
            if (message.author !=
                    client.user) & (message.author.display_name[-3:] != "@運営"):

                # コマンドに対応するデータを取得して表示
                username, database_id, curr_number, err_announce = get_message(
                    message)
